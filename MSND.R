...
rm(list = ls())
#set workspace
setwd("E:/gse81861/filter/screen");
...
allpar <- as.data.frame(matrix(nrow = length(data), ncol = 7))

for(i in 1:length(data)){
  subname <- substr(names(data)[i], 1, nchar(names(data)[i])-4)
  row.names(allpar)[i] <- subname
}
colnames(allpar) <- c("alpha", "beta", "gamma", "delta", "miu", "sigma", "theta")

#set t_points and u_points to estimate alpha, beta, gamma and delta
...
for(m in 1:length(data)){
  ...
  ep <- fitdistr(xxve, densfun = "normal") 
  allpar[m, 5] <- round(ep[["estimate"]][["mean"]], 6)
  allpar[m, 6] <- round(ep[["estimate"]][["sd"]], 6)
}
...
den <- c(dstable(s1, p1, p2, p3, p4), theta*dnorm(s2, p5, p6))
...
#output parameter
outname <- paste(paste("E:/gse81861/filter/parameter/p", UTpoints, sep = ""), ".csv", sep = "")
